...
const {IMAGE,Authorization,unexpired,routes}=tools.components, {LIST_UNXPIRED}=tools.components.services, {img_products}=unexpired;
...
export class UnexpiredComponent implements OnInit {
  img_path=IMAGE+img_products
   auth!:string;
   products!:Product[]
   constructor(private net:UnexpiredService,private nav:Router) {
